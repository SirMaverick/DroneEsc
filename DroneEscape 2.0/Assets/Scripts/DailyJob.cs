...
public class DailyJob : MonoBehaviour {
	...
	void Start () {
        ...
        inScreenSizeX = Screen.width / 3;
        inScreenSizeY = Screen.height / ( maxAmount * collumns + 4.0f);
        height = inScreenSizeY;
        ...
        if(collumns % 2 == 1) {
            for (int x = -maxAmountFixed + 1; x < maxAmountFixed; x++) {
                ...
                for (int y = -maxAmountFixed + 1; y < maxAmountFixed; y++) {
                    ...
                    if (y  == -maxAmountFixed + 1) {
                        temp = arrayList[0];
                        temp[xFixed, yFixed] = new Vector3(transform.position.x + x * width, transform.position.y + yFixed * height + (-maxAmountFixed) * maxAmount * height + 2 * height, transform.position.z);
                        arrayList[0] = temp;
                        temp = arrayList[arrayList.Count - 1];
                        temp[xFixed, yFixed] = new Vector3(transform.position.x + x * width, transform.position.y + yFixed * height + (-maxAmountFixed + 4) * maxAmount * height + 2 * height, transform.position.z);
                        arrayList[arrayList.Count - 1] = temp;
                    }
                    for (int i = 1; i < arrayList.Count - 1; i++) {

                        temp = arrayList[i];
                        temp[xFixed, yFixed] = new Vector3(transform.position.x + x * width, transform.position.y + yFixed * height + (-maxAmountFixed + i) * maxAmount * height, transform.position.z);
                        arrayList[i] = temp;
                    }
                }
            }
            ...
            for (int x = -maxAmountFixed; x < maxAmountFixed; x++) {
                ...
                for (int y = -maxAmountFixed; y < maxAmountFixed; y++) {
                    ...
                    for (int i = 1; i < arrayList.Count - 1; i++) {
                        temp = arrayList[i];
                        temp[xFixed, yFixed] = new Vector3(transform.position.x + x * width + width / 2, transform.position.y + yFixed * height + (-maxAmountFixed + i - 1) * maxAmount * height + height / 2, transform.position.z);
                        arrayList[i] = temp;
                    }
                }
            }
